...
export interface InnerRegisteredFunc<F extends Func> {
  ...
  beginRun: (
    id: string,
    args: Params<F>,
    cb: (invocationHandler: InvocationHandler<Ret<F>>) => void,
  ) => void;
}
...
export class ResonateInner {
  ...
  constructor(
    network: Network,
    config: { pid: string; group: string; ttl: number },
  ) {
  ...
  public beginRun<T>(
    ...
    this.network.send(
      ...
      (timeout, res) => {
        ...
        if (!task) {
          console.log(
            "got no task, should create a sub for the promise instead",
          );
        ...
        compu.invoke(task, { id: durable.id, fn: func, args }, (_err, result) =>
  ...
  public register<T extends Func>(
    name: string,
    func: T,
  ): InnerRegisteredFunc<T> {
    ...
    return {
      beginRun: (
        id: string,
        args: Params<T>,
        cb: (invocationHandler: InvocationHandler<Ret<T>>) => void,
      ) => {
        ...
        (_timeout, response) => {
          if (response.kind === "claimedtask") {
            ...
            if (!func) {
              ...
              console.log(
                "got a task that can not be executed in this node, dropping task",
              );
              return;
            }
